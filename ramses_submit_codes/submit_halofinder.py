...
# Base command for running the gas_analysis script
...
job_script_dir = "/user1/poojarani/Lya_Comparison/ramses_data/lev{lev:03}_len{length}/Halo_data/job_scripts/".format(
    lev=lev, length=lengths)
os.makedirs(job_script_dir, exist_ok=True)

# Define the log directory for .o and .e files
log_dir = "/user1/poojarani/Lya_Comparison/ramses_data/lev{lev:03}_len{length}/Halo_data/logs/".format(
    lev=lev, length=lengths)
...
for output_number in output_numbers:
    ...
    command = base_command.format(output=output_number, length=lengths, lev=lev)

    # Create a unique job script name. For clarity, we use the same base for log files.
    job_script_name = f"HF_{output_number}_{lev}_{lengths}.pbs"
