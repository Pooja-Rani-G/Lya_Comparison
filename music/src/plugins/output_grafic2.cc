  ...
  //float metal_floor_;
    ...
    void write_refinement_mask( const grid_hierarchy& gh )
    {
        ...
        // do finest level
        {
            ...
            // write mask
            sprintf(ff,"%s/lev%03d_len%d/ic_refmap",fname_.c_str(), gh.levelmax(),static_cast<int>(boxlength) );
	    ...
	    if( passive_variable_value_ > 0.0f )
	      {
            sprintf(ff,"%s/lev%03d_len%d/ic_pvar_%05d",fname_.c_str(), gh.levelmax(), static_cast<int>(boxlength),passive_variable_index_ );
	    ...
	    if( passive_variable_value_ > 0.0f )
	      {
		sprintf(ff,"%s/lev%03d_len%d/ic_pvar_%05d",fname_.c_str(), ilevel,static_cast<int>(boxlength), passive_variable_index_ );
		ofs_metals.open(ff,std::ios::binary|std::ios::trunc);
...
public:
	...
	: output_plugin( cf )
	{
		...
		{
			char fp[256];
			sprintf(fp,"%s/lev%03d_len%d",fname_.c_str(), ilevel,static_cast<int>(boxlength) );
			mkdir( fp, 0777 );
		}
	...
	void write_dm_position( int coord, const grid_hierarchy& gh  )
	{
		...
		for(unsigned ilevel=levelmin_; ilevel<=levelmax_; ++ilevel )
		{
			...
			char ff[256];
			sprintf(ff,"%s/lev%03d_len%d/ic_posc%c",fname_.c_str(), ilevel,static_cast<int>(boxlength), (char)('x'+coord) );
	...
	void write_dm_velocity( int coord, const grid_hierarchy& gh )
	{
		...
		for(unsigned ilevel=levelmin_; ilevel<=levelmax_; ++ilevel )
		{
			...
			char ff[256];
			sprintf(ff,"%s/lev%03d_len%d/ic_velc%c",fname_.c_str(), ilevel,static_cast<int>(boxlength), (char)('x'+coord) );
	...
	{	
		...
		for(unsigned ilevel=levelmin_; ilevel<=levelmax_; ++ilevel )
		{
			...
			char ff[256];
			sprintf(ff,"%s/lev%03d_len%d/ic_velb%c",fname_.c_str(), ilevel,static_cast<int>(boxlength), (char)('x'+coord) );
	...
	{	
		...
		for(unsigned ilevel=levelmin_; ilevel<=levelmax_; ++ilevel )
		{
			...
			char ff[256];
			sprintf(ff,"%s/lev%03d_len%d/ic_deltab",fname_.c_str(), ilevel ,static_cast<int>(boxlength));
