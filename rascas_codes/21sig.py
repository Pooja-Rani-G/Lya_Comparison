...
#python3 21cmsig.py -ram lev6_len100/output_00005/ -ras lev6_len100/trilin_interp/ -hf outram/lev10_len100/info_00011/info_00011.0.h5

#/user1/shikhar/outram/lev10_len200/info_00005/info_00005.0.h5
...
workdir = '/user1/poojarani/Lya_Comparison/'

#Do you want the star formation to affect instantaneously?
instantaneous = False
...
ras = workdir+f"rascas_data/{length}cMpc_{2**lev_hi}/{length}cMpc_{2**lev_lo}/output_{halo_snap:03}/p1e{exp_of_photons}/Pa.dat"
print('\nReading RASCAS data from',ras,' ...')
f = ff(ras)
[lev] = f.read_ints()
raw = f.read_reals('d')
np.nan_to_num(raw,copy=False)	
print("level of palpha is",lev)
...
info_file=np.loadtxt(workdir+f"rascas_data/{length}cMpc_{2**lev_hi}/{length}cMpc_{2**lev_lo}/output_{halo_snap:03}/info.txt",comments="#")
gas_num=int(info_file[1][1])
print("gas_num is", gas_num)
ram = workdir+f"ramses_data/lev{lev_lo:03}_len{length}/output_{gas_num:05}"
...
def vel_grad(zed,vz):
	'''
	This is the peculiar velocity gradient term only along the z axis.
	Input z component of velocity.
	Dimensionless.
	'''
	Dvz = np.gradient(vz,axis=2)
	DvzDr = (1+zed)*Dvz/Del_rc
	return (1+DvzDr/H(zed))**-1
...
def sig21cm(zed,Tk,nHI,ne,npr,palpha,vz):
	'''
	This is the 21-cm signal with multiple scattering.
	Also, note that the velocity gradient term is only for the z component.
	LOS is z axis.
	'''
	xa=xalpha(palpha)
	x_coll=xk(Tk,nHI,ne,npr)
	xcmb,Ts=xcmb_Ts(zed,Tk,x_coll,xa,nHI)
	return 1000*3*lam21**3*A_10*Tstar/(32*np.pi*H(zed)*(1+zed))*nHI*(1-Tcmb/Ts)*vel_grad(zed,vz)	#in mK
#--------------------------------------------------------------------------------------------

#============================================================================================

fn = workdir+f'ramses_data/lev{lev_hi:03}_len{length}/Halo_data/info_{halo_snap:05}/info_{halo_snap:05}.0.h5' 
print('\nReading halo data from',fn,' ...\n')
dsh = yt.load(fn)
adh = dsh.all_data()             
M_haloes = adh['halos','particle_mass']
M_tot = np.sum(M_haloes.d)
...
def SFRD(zed):	#in kg.m^-3.sec^-1
	...
	dFbydz=(f_coll(zed+1e-3)-f_coll(zed))*1e3
...
def J_alpha(zed):
	...
	if instantaneous == True:
		for i in range(n_haloes):
			...
			ith_halo_mass = ith_halo.mass.d
			sfr_i_by_Vcell = 8**lev*SFRD(zed)*ith_halo_mass/M_tot
			...
			rho_star_dot[ix,jy,kz] = rho_star_dot[ix,jy,kz] + sfr_i_by_Vcell
	
	else:
		for i in range(n_haloes):
			ith_halo = dsh.halo("halos",i) #loading the ith halo
			ith_halo_mass = ith_halo.mass.d #ith halo mass in Msun
			ith_halo_pos = ith_halo.position.in_units("code_length").d  #ith halo position in code units
			ith_halo_pos_wrt_centre = L_m_co*(ith_halo_pos-0.5)  #ith halo position in comoving metre w.r.t. box centre
			ith_halo_dis_wrt_centre = np.sqrt(np.sum(ith_halo_pos_wrt_centre**2))
			z_prime = zprime(zed,ith_halo_dis_wrt_centre)
                
			sfr_i_by_Vcell = 8**lev*SFRD(z_prime)*ith_halo_mass/M_tot
			ix,jy,kz = np.int_(ith_halo_pos/Del_r_code_units)
			rho_star_dot[ix,jy,kz] = rho_star_dot[ix,jy,kz] + sfr_i_by_Vcell
...
def xalpha_old(zed,nHI,Tk):
	...
	a=4.71e-4/np.sqrt(Tk*1e-4)	#Natural to Doppler width
...
def sig21cm_old(zed,Tk,nHI,ne,npr,vz):
	'''
    This is the 21-cm signal without multiple scattering.
    Also, note that the velocity gradient term is only for the z component.
	LOS is along z axis.
    '''
...
#----------------------------------------------------------------------------------------------
output_dir = workdir+f"rascas_data/{length}cMpc_{2**lev_hi}/{length}cMpc_{2**lev_lo}/output_{halo_snap:03}/p1e{exp_of_photons}/"

# Your filename
output_file = open(output_dir+"coupling_info.txt", 'w')
...
with redirect_stdout(tee):
    ...
    T21_new = sig21cm(cr_rdshft,Tk,nHI,ne,npr,palpha,vz)
    ...
    T21_old = sig21cm_old(cr_rdshft,Tk,nHI,ne,npr,vz)
...
print('Saving 21-cm signal and Lyman-a coupling in ...',output_dir)

np.save(output_dir+'T21',T21_new)
np.save(output_dir+'xa',xa_new)
