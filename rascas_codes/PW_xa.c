//Compile as gcc PW_xa.c -std=gnu99 -I/mnt/exports/data/install/fftw-3.3.8/include -L/mnt/exports/data/install/fftw-3.3.8/lib -lfftw3 -I/apps/gsl-2.7.1/include -L/apps/gsl-2.7.1/lib -lgsl -lgslcblas -lm -o psxa
//Run as ./psxa {lev} {lev_lo} {length} {output_forlder} {photon_no}
...
void myps(int nbox, float **grid, float box_size, int outs,int ncell_high, int nphoton)
{
  ...
  // ----------------------
   ...
   char save_path[MAX_PATH_LENGTH];
    snprintf(save_path, MAX_PATH_LENGTH,  "/user1/poojarani/Lya_Comparison/rascas_data/%03dcMpc_%d/%03dcMpc_%d/output_%03d/p1e%d/xaPS.txt", (int)box_size, ncell_high, (int)box_size, nbox, outs,nphoton);
    printf("Saving to: %s\n", save_path);  // <-- This will print the path
...
{
    if (argc < 6) {
        fprintf(stderr, "Usage: %s <lev> <lev_lo> <length> <outputno> <nphoton>\n", argv[0]);
        return EXIT_FAILURE;
    }
    int lev = atoi(argv[1]); // Higher Resolution
    int lev_lo = atoi(argv[2]); // Downsampled Resolution 
    float boxsize = atof(argv[3]); // boxsize in Mpc/h
    int outputno = atoi(argv[4]); // Snapshot number
    int nphoton = atoi(argv[5]); // No of MC photons= 10**nphoton  

   int ncell = pow(2,lev_lo);		//no.of cells in each direction
   int ncell_high = pow(2,lev); 
   ...
   char file_path[MAX_PATH_LENGTH];
    snprintf(file_path, MAX_PATH_LENGTH, "/user1/poojarani/Lya_Comparison/rascas_data/%03dcMpc_%d/%03dcMpc_%d/output_%03d/p1e%d/delta_xa.bin", (int)boxsize, ncell_high, (int)boxsize, ncell, outputno, nphoton);
    // Print the filename
    printf("Filename: %s\n", file_path);
   ...
   myps(ncell,&del21,boxsize,outputno,ncell_high,nphoton);	//This is the main command where the PS is calculated.
