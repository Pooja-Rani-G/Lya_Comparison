...
!========================================================================================================  
  !--------------------- read number of levels and z ---------------------
  if (verbose .and. rank==0) print*,'Shikhar: reading base level and the redshift of the snapshot...'
  open(unit=20,file=trim(DomDumpDir)//trim('raminfo.txt'),status='old',form='formatted')
  read(20,*)lev
  read(20,*)one_plus_z
  close(20)
  
  if (verbose .and. rank==0) print*,'Shikhar: base level and redshift of your RAMSES simulation:', lev,one_plus_z-1
!========================================================================================================    
  ...
  if (rank == 0) then
     ...
     call master(file_compute_dom, ndomain, domain_file_list, PhotonICFile, nbundle, fileout,lev, pa_fileout) !Shikhar: note the extra parameters 'lev' and 'pa_fileout'.
  else
     ! Worker section, will mostly do radiative transfer (MCRT)
     call worker(file_compute_dom, ndomain, mesh_file_list, nbundle,lev,one_plus_z)   !Shikhar: note the extra parameters, lev and one_plus_z.
  end if
