module module_ramses
  ...
  public :: get_param_real      !Shikhar: I added this for use in CreateDomDump.
  !==================================================================================
contains
...
!Shikhar: I made this function public for its use in CreateDomDump.
...
!Shikhar: changes made in the following function.
  subroutine get_fields_from_header(dir,ts,nfields)
    ...
    read(50,*)  !header
    read(50,*)  !other_tracer 
    read(50,*)  !debris_tracer
    read(50,*)  !cloud_tracer
    read(50,*)  !star_tracer
    read(50,*)  !other_tracer
    read(50,*)  !gas_tracer
    read(50,*)  !DM
...
!========================================================================================
!Shikhar: I added these two fields which were missing originally.  
    integer(kind=1),allocatable            ::fam(:), tag(:)
!========================================================================================
    ...
    if (nstars == 0) then
       ...
       stop
!=============================================================================
!Shikhar: added an informative print statement about the total number of stars available in your full simulation box.
    else
       write(*,*) 'Shikhar: total no.of star from RAMSES simulation = ',nstars
       print*
    end if
!=============================================================================
...
!=============================================================
       allocate(fam(1:npart),tag(1:npart))
!=============================================================
       ...
       do ifield = 1,nfields
          ...
          case('level')
             read(iunit)
!==============================================================
!Shikhar: Added these missing fields.
          case('family')
             read(iunit) fam(1:npart)
          case('tag')
             read(iunit) tag(1:npart)
!==============================================================
          case('tform')
       ...
       ilast = 0
       do i = 1,npart
!Shikhar: In the following 'if' I added an extra condition to be a star, which is more accurate.
          if (age(i).ne.0.0d0 .and. fam(i)==2) then ! This is a star.
             temp(:) = x(i,:)
       ...
       deallocate(age,m,x,id,mets,v,imass,fam,tag)
...
!Shikhar: I modified this function so that it is compatible with the new RAMSES header_*.txt files.
  function get_tot_nstars(dir,ts)
    ...
    read(50,*)  !gas_tracer
    read(50,*)  !DM
    read(50,*) fieldname, get_tot_nstars
    close(50)
