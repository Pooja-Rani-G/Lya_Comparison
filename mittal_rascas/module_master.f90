...
!Shikhar: note the extra parameters 'lev' and 'pa_fileout'
  subroutine master(file_compute_dom, ndomain, domain_file_list, file_ICs, nbundle, fileout, lev, pa_fileout)
    ...
    integer(kind=4)                               :: i,j,icpu,idcpu,jnewdom,ncpuended,ntest
...
!======================================================================================================    
    integer(kind=4)                               :: k, nphottodo, nphotdone
    integer(kind=4),intent(in)  :: lev
    character(2000),intent(in)  :: pa_fileout
    real(kind=8), allocatable   :: palpha(:,:,:), temp_palpha(:,:,:)

    allocate(palpha(2**lev,2**lev,2**lev),temp_palpha(2**lev,2**lev,2**lev))

!'temp_palpha' is palpha for nbundle no.of photons received from each worker. Final palpha of our interest -- 'palpha'
...
!=====================================================================
!Shikhar: updated the following command with 'lum' parameter. It is the total box luminosity by number.
       call init_photons_from_file(file_ICs,photgrid, lum)
!=====================================================================
       nphot = size(photgrid)   !Shikhar: this is the total number of MC photons used for simulation.
       nphottodo = nphot
    ...
    delta(:)=0.0d0
    do k=1,nphot
       if(photgrid(k)%status==0)then  ! for restart
          j = get_my_new_domain(photgrid(k)%xcurr,domain_list)
          call add_photon_to_domain(k,j)
       endif
    enddo
    ...
    do icpu=1,nworker
       ...
       j=cpu(icpu)
       !if(verbose) print '(" [master] allocates domain ",i5," to cpu ",i5)',j,icpu  !!Shikhar: commented this for brevity.
    ...
    do while(everything_not_done)
       ...
       call MPI_RECV(ntest, 1, MPI_INTEGER, MPI_ANY_SOURCE, MPI_ANY_TAG, MPI_COMM_WORLD, stts, IERROR)
       idcpu = stts(MPI_SOURCE)

       call MPI_RECV(photpacket(1)%id, nbundle, MPI_TYPE_PHOTON, idcpu, DONE_TAG, MPI_COMM_WORLD, stts, IERROR)


!================================================================================================   
!       print*,'[master] (Shikhar) waiting for palpha from CPU',idcpu
!       receiving palpha for nbundle no.of photons from a worker
       call mpi_recv(temp_palpha,8**lev,mpi_double_precision,MPI_ANY_SOURCE,4,MPI_COMM_WORLD,stts,IERROR)
!       print*,'[master] (Shikhar) received palpha from CPU',idcpu
       palpha = palpha + temp_palpha
!================================================================================================
...
!==================================================================
!Shikhar: temporarily skipping this to save time and memory.
...
!Shikhar: commented this message for brevity.
...
!!The end of above loop marks the end of MCRT for all photons. The palpha due to each photon has been summed into
!!the variable 'palpha'. Not yet converted to actual meaningful palpha.
...
!Shikhar: in our work we don't really these properties. So to save space you can comment the following.
...
!=======================================================================================================
  palpha = lum/nphot*palpha

  !Master node will save the palpha. Saving palpha in the file 'pa_fileout'.

  if(verbose) print *,'[master] (Shikhar) now writing palpha data to ',trim(pa_fileout)
  open(unit=27, file=pa_fileout, form='unformatted', action='write')
  write(27) lev
  write(27) palpha
  close(27)
  deallocate(temp_palpha,palpha)
  if(verbose) print *,'[master] (Shikhar) Done. Enjoy!'
!=======================================================================================================
  ...
  subroutine print_diagnostics
    ...
    if(verbose)then
       ...
       !=======================================================================================
       !Shikhar: added this extra diagnostic
       print *,'   min max tau         =',minval(photgrid%total_OD),maxval(photgrid%total_OD)
       !=======================================================================================
    endif
