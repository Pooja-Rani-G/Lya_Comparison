module module_parallel_mpi
  ...
  integer(kind=4),dimension(MPI_STATUS_SIZE)      :: stts               !To avoid confusion with the keyword I
                                                                        !changed this to stts

  ! define a MPI derived type for photons
  integer(kind=4),parameter                       :: nbloc=11   !!Shikhar: earlier it was 10. 1 extra for total_OD.
...
!===========================================================================================
    ...
    types = (/ MPI_INTEGER, &
         ...
         MPI_DOUBLE_PRECISION, &
         MPI_INTEGER, &
         MPI_DOUBLE_PRECISION /)  !!Shikhar: added this corresponding to the "total_OD"

    ! block lengths
    !longueurs_blocs = (/1,1,3,3,1,3,1,1,1,1/)
    longueurs_blocs = (/1,1,3,3,1,3,1,1,1,1,1/)  !!Shikhar: added an extra corresponding to "total_OD"
...
!==========================================================================    
    call MPI_GET_ADDRESS(p(1)%total_OD,     adresses(11),code)
!==========================================================================
  ...
  subroutine test_mpi_type
    ...
    if (rank == 1) then
       call MPI_RECV(temp_p(1)%id, 10, MPI_TYPE_PHOTON, 0, MPI_ANY_TAG, MPI_COMM_WORLD, stts, IERROR)
       do i=1,10
