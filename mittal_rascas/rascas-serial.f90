...
!====================================================================================================  
  integer(kind=4)           :: lev
  real(kind=8)              :: one_plus_z, lum
  real(kind=8), allocatable :: palpha(:,:,:)
  character(2000)           :: pa_fileout = 'palpha.dat'   ! output file for scattering rate 
!====================================================================================================  
  ...
  call init_photons_from_file(PhotonICFile,photgrid, lum)   !!Shikhar: added the total luminosity (total #/s)
  nphot = size(photgrid)  !!nphot is the number of MC photons used for simulation.
  if (verbose) print *,'--> Nphoton =',nphot
  ! ------------------------------------------------------------
  
!========================================================================================================  
  !--------------------- Shikhar: read number of levels ---------------------
  print*
  print*,'Shikhar: reading number base level and redshift...'
  open(unit=20,file=trim(DomDumpDir)//trim('raminfo.txt'),form='formatted')
  read(20,*)lev
  read(20,*)one_plus_z
  close(20)
  print*,'Shikhar: base level and redshift of your RAMSES simulation = ',lev, one_plus_z-1
  print*
  allocate(palpha(2**lev,2**lev,2**lev))	!!Shikhar
...
!!SHIKHAR: THIS IS THE MAIN STEP!
  ! do the Monte Carlo Radiative Transfer
   call MCRT(nphot,photgrid,meshdom,compute_dom,palpha,lev,one_plus_z)  !!Shikhar: Added 3 more argument; 'lev','palpha', and 1+z
   !call MCRT(nphot,photgrid,meshdom,compute_dom,palpha,lev)  !Use this for non-Hubble flow
...
!=======================================================================================================
!Shikhar: writing palpha in the same directory as domain dump files.
  palpha = lum/nphot * palpha

  if(verbose) print *,'Shikhar: now writing palpha data to',trim(pa_fileout)
  open(unit=27, file=pa_fileout, form='unformatted', action='write')
  write(27) lev
  write(27) palpha
  close(27)
  deallocate(palpha)
  if(verbose) print *,'Shikhar: done. Enjoy!'
!=======================================================================================================
contains
...
!====================================================================
  ...
  subroutine print_serial_params(unit)
    ...
    if (present(unit)) then 
       ...
       write(unit,'(a,a)')           '  pa_fileout     = ',trim(pa_fileout)        !Shikhar.
       ...
       write(*,'(a,a)')           '  pa_fileout     = ',trim(pa_fileout)           !Shikhar.
